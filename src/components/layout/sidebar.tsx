...
export function Sidebar({ className, onClose }: { className?: string; onClose?: () => void }) {
  ...
  return (
    <aside className={cn("hidden w-[234px] shrink-0 border-r border-border/70 bg-[#f5f9ff]/85 backdrop-blur lg:flex lg:flex-col", className)}>
      <div className="flex h-16 items-center justify-between border-b border-border/70 px-4">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="rounded-md p-1.5 text-[#54708b] hover:bg-black/5 lg:hidden"
